...
for(var i = 0; i < 12; i += 1) {
    ...
    if (i == buttonSelect) {
		...
		if (settingButton)  {
            draw_text(xSelector+xSeperation,ySelector+(i*ySeperation)+32,global.texts[? "pressButtonToSet"]);
        }
    }
	
	// Show the reset controls prompt
    if (resetPrompt && i == 11) {
        draw_text(xSelector+xSeperation,ySelector+(i*ySeperation)+32,global.texts[? "controlsReset"]+"!");
    }
}
